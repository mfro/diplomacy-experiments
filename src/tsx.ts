...
export default function jsx(name: string, props: any[], ...children: any[]) {
    ...
    if (children) {
        for (let childArg of children) {
            let child;

            if (childArg instanceof Node)
                child = childArg;
            else if (typeof childArg == 'string')
                child = document.createTextNode(childArg);
            else {
                debugger;
                continue;
            }
